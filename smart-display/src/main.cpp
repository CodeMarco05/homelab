...
#include "weather_manager.h"  // Weather management functions
...
void setup() {
  ...
  if (lvgl_port_lock(-1)) {
    ...
    initWeatherChart(objects.weather);
...
void loop() {
  ...
  // Update weather chart and weekly forecast every 5 minutes (300000ms)
  if (millis() - lastWeatherUpdate >= 300000 || lastWeatherUpdate == 0) {
